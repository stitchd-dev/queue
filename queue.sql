-- Event queue schema with partitioned datasets for high-throughput ingestion.
-- Provides helper functions for dataset management, reservation handling, and job status updates.
--
-- Cleanup
...
-- Queue metadata table tracking dataset state and reservations
create table queue
(
    -- Queue identifier
...
-- Creates dataset partition with data and job tables
...
begin
    ...
    -- Job table for processing state
    execute format(
            'create table %I (
                id         serial primary key,
...
-- Creates a new queue with initial dataset and returns queue ID
...
-- Composite type for batch updating failed jobs with retry timestamps
...
-- Batch updates job statuses (done, failed, failed_permanently) after processing
...
-- Returns current dataset for insertion, creates new dataset if capacity reached
...
-- Releases reserved slots after data insertion completes
